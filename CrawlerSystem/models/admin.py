...
from CrawlerSystem.settings import get_app_label  
...
class AdminManagerMixin(object):
    ...
    def get_admin(cls,admin_id):
        '''获取管理员
        '''
        admin_objs =  cls.objects.filter(id=admin_id,status=cls.Status.NORMAL)
        if admin_objs:
            return admin_objs[0]
...
class Admin(models.Model,BaseModelMixin,AdminManagerMixin):
    ...
    def md5_password(self,password_str=''):
        return hashlib.new('md5', password_str + 'crawler').hexdigest() if password_str else self.password 
