...
class AuthMiddleware(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if hasattr(view_func,'notauth') or view_func.__module__ == 'django.views.static' :#不需处理的函数不管
            ...
            if  (request.path_info.find('/login') == 0 and request.method == 'POST'):
                request.admin = Admin()
                request.admin.id = 0
                self.save_operate_log(request, 'login system')
        else:
        ...
        for k,v in params.iteritems():
            if type(v) == list and v.__len__() == 1:
                _params[k] = v[0]
            _params[k] = v
    ...
    def check_url_permsssion(self,request):
        ...
        for k,menu in request.allow_menu.iteritems():
                if  url_path == menu.url_path and menu.is_match_url_parmas(_params) :
                    match_menu = menu
                    is_allow = True
                    break
    ...
    def save_operate_log(self,request,msg,log_type=29):
        ...
        try:
            ...
            log.log_data =  0
            log.log_name = request.POST["username"] if request.POST["username"] else ''
            log.log_result = 0
            log.f1 = msg[:100]
