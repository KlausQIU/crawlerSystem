...
from CrawlerSystem.models.menu import Menu
import json
import sys
reload(sys)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        fileDir = r'C:\Users\moonq\Google driver\python\07crawler\crawlerSystem\CrawlerSystem\menu.json'
        # if len(args) == 0:
        #     sys.stdout.write(u'缺少参数: 请输入菜需要创建的菜单数据文件.')
        #     return
        # else:
        #     fileDir = args[0]
        #     if not isValidFiledir(fileDir):
        #         sys.stdout.write(u'错误: 无效的文件路径.')
        #         return
        b = "./menuu.json"
        menu_json = open(fileDir)
        c = json.loads(menu_json.read())
        ...
        for i in c:
            try:
                ...
                selfId, parentId, order = self.getIdAndParentId(von)
                name = i[1]
                if len(i) < 3:
                    url = "javascript:;"
                else:
                ...
                if len(i) >= 4:
                    menu_type = int(i[3]) if i[3] or i[3] == 0 else None
