...
from settings import TEMPLATE_DIRS, DATABASES,STATICFILES_DIRS
...
def get_static_root(*child_dir_names):
    the_dir =  os.path.join(STATICFILES_DIRS[0],*(str(x) for x in child_dir_names))
    mkdir(the_dir)
    return the_dir
...
class GlobalPathCfg(object):
    ...
    def get_notice_html_save_path(self, file_name):
        static_path = self.get_static_folder_path()
        path = os.path.join(  STATICFILES_DIRS[0],'notice','html')
        mkdir(path)
