;(function($,window,document,undefined){
	...
	EventFlow.prototype = {
		...
		spliceHtml :function(element,options){
			...
			for(var i = 0;i<options.events.length;i++){
				...
				list +='<span class="big_squre"><i>'+options.events[i].time+'</i></span>';
                // list += '<span class="small_squire"><i></i></span>'

                list += '<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;操作:'+options.events[i].even+'</p>'
                    + '<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;访问地址:' +options.events[i].url + '</p>'
                    + '<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;访问IP:' + options.events[i].ip + '</p>'
                    +'</div>'; 
    ...
    //                     + '<p>' + options.events[i].events[j].url + '</p>'
				// 	+'</div>';				
				// }
				list += '</div>';
			};
